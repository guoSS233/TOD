...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			if(v.vTexCoord.x < pointOne && (hunDisp > 0 || commaVal > 0)) {
				float xPoint = (0.1f * hunDisp) + v.vTexCoord.x;
				return tex2D( MapTexture, float2(xPoint, v.vTexCoord.y));
			} else if(v.vTexCoord.x > pointOne && v.vTexCoord.x < pointTwo && (hunDisp > 0 || tenDisp > 0 || commaVal > 0)) {
			...
			} else if(v.vTexCoord.x > basePoint) {
