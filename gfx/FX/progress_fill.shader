...
PixelShader =
{
	...
	[[
		...
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			if( (v.vTexCoord0.x <= CurrentState) && ( CurrentState >= 0.5 ) )
				return float4(progressColor1.r, progressColor1.g, progressColor1.b, progressColor1.a); 
			else if ( (v.vTexCoord0.x <= CurrentState) && ( CurrentState < 0.5 ) )
				return float4(progressColor2.r, progressColor2.g, progressColor2.b, progressColor2.a);
			else
